...
export default function CursorRing() {
    ...
    const curr = React.useRef({ x: 0, y: 0 });
    const down = React.useRef(false);
    const raf = React.useRef<number>(0);

    React.useEffect(() => {
        // safe to read window here
        target.current = { x: window.innerWidth / 2, y: window.innerHeight / 2 };
        curr.current = { ...target.current };
        ...
        const tick = () => {
            ...
            if (ref.current) {
                ref.current.style.transform =
                    `translate3d(${curr.current.x}px, ${curr.current.y}px, 0) translate(-50%, -50%) scale(${down.current ? 0.85 : 1})`;
            }
